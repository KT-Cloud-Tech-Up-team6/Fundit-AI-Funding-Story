...
def plan(project: ProjectInput, review: Review):
    if len(review.problems) != 4 or len(review.strengths) < 3 or not project.rewards or not project.asset_ids:
        raise ValueError("불편한 상황 4개, 핵심 강점 3개 이상, 선물과 제품 이미지를 확인해 주세요.")
    blocks = [copy.deepcopy(CATALOG["hero"]), copy.deepcopy(CATALOG["problem"])]
    for index, strength in enumerate(review.strengths):
        ...
        blocks.append(block)
    blocks.append(copy.deepcopy(CATALOG["rewards"]))
    fixed = {}
    for i in range(3):
    ...
    for block in blocks:
        for node in block["nodes"]:
            ...
            if node["kind"] == "image":
                ...
                node["assetId"] = ""
                node["pending"] = True
    scene = {**TEMPLATE["scene"], "brand": project.brand_color, "blocks": blocks}
    return scene, fixed
