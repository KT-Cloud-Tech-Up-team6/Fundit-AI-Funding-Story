...
| 공통 크라우드 펀딩 안내 | Fundit 고정 문구를 코드에서 삽입; LLM 생성 대상 아님 |
...
| 마지막 `TEXT` | 확인된 항목의 예산 → 일정 → 팀 → 신뢰와 안전 순서. 안전 하위는 고정 펀딩 안내 → 선택적 정책 → 선택적 어려움. 선택 정보가 모두 비어 있어도 고정 안내는 포함 |
| 생성 HTML | `section`, `h2`, `h3`, `hr`, `p`, `strong`, `br`; `h2`의 왼쪽 세로선과 구분선 등 코드 소유의 고정 `style`만 사용. 사용자 입력 값 전체 escape, 링크·스크립트 없음 |

전송은 기존 `IMAGE/TEXT` DTO를 유지한다. 단일 `html` 필드를 추가하거나 `<img>`를 `TEXT`에 넣지 않는다.
BE가 검증한 이미지 URL과 HTML을 하나의 본문으로 표시·편집하는 처리는 FE 연동 대상이다.
BE/FE는 새 태그와 코드에서 지정한 정적 `style` 속성의 보존·안전한 렌더링을 확인해야 한다. 이를 지원하지 않는 경계에서는 상위 제목의 세로선과 섹션 계층이 보존되지 않는다.
고정 펀딩 안내는 Fundit 서비스 정책 확인 전 법률·환불 약관으로 사용하지 않는다. 리워드 접기 UI는 AI HTML에 넣지 않는다.
향후 사용자 지정 하단 문구 기능을 도입하면 기존 섹션 뒤 최하단에 배치한다. 이번 범위에는 포함하지 않는다.
